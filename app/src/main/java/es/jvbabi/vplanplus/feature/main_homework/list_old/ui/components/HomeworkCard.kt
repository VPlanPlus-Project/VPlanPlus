...
import es.jvbabi.vplanplus.feature.main_homework.list_old.ui.components.homeworkcard.AddHomeworkTask
import es.jvbabi.vplanplus.feature.main_homework.list_old.ui.components.homeworkcard.DraggableHost
import es.jvbabi.vplanplus.feature.main_homework.list_old.ui.components.homeworkcard.HomeworkProgressBar
import es.jvbabi.vplanplus.feature.main_homework.list_old.ui.components.homeworkcard.HomeworkTask
...
fun HomeworkCard(
    ...
    AnimatedVisibility(
        visible = (showHidden || ((homework as? Homework.CloudHomework)?.isHidden == false) || homework is Homework.LocalHomework)  && (showDone || homework.tasks.any { !it.isDone }),
        enter = expandVertically(tween(250)),
        exit = shrinkVertically(tween(250))
    ) {
        Box(modifier.clickable { onClick() }) {
            DraggableHost(
                iconLeft = if (isOwner || homework is Homework.LocalHomework) Icons.Outlined.Delete else Icons.Outlined.VisibilityOff,
                ...
                onDragToLeft = {
                    if (isOwner || homework is Homework.LocalHomework) onDeleteRequest()
                    else onHomeworkHide()
                },
                ...
                Column(
                    ...
                    RowVerticalCenterSpaceBetweenFill {
                        ...
                        HeaderActions(
                            isLoading = isLoading,
                            isHomeworkHidden = (homework as? Homework.CloudHomework)?.isHidden ?: false,
                            isHomeworkPublic = (homework as? Homework.CloudHomework)?.isPublic ?: false,
                            userCanEditHomework = isOwner || homework is Homework.LocalHomework,
                            isLocalHomework = homework is Homework.LocalHomework,
                    ...
                    homework.tasks.forEach { task ->
                        HomeworkTask(
                            ...
                            isLoading = isLoading,
                            isDone = task.isDone,
                            userCanEditThisTask = isOwner || homework is Homework.LocalHomework,
                            ...
                            val text = buildAnnotatedString {
                                ...
                                withStyle(style) {
                                    ...
                                    withStyle(style.copy(color = MaterialTheme.colorScheme.onSurfaceVariant)) {
                                        if (homework is Homework.LocalHomework) append(stringResource(id = R.string.homework_thisDevice))
                                        else if (isOwner) append(stringResource(id = R.string.homework_you))
                                        else append((homework as Homework.CloudHomework).createdBy.name)
                                    }
                                }
                            }
                            Text(text)
                            if (isOwner && (homework as? Homework.CloudHomework)?.isPublic == true) Icon(modifier = Modifier.size(12.dp), imageVector = Icons.Default.Share, contentDescription = null)
                            if ((homework as? Homework.CloudHomework)?.isHidden == true) Icon(modifier = Modifier.size(12.dp), imageVector = Icons.Default.VisibilityOff, contentDescription = null)
                        }
                    }
                }
            }
