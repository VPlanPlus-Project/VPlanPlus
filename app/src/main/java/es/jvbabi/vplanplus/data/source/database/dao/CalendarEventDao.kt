...
abstract class CalendarEventDao {
    ...
    @Query("SELECT * FROM calendar_events WHERE date = :date AND schoolCalendarEventRefId = :schoolId")
    ...
    @Query("DELETE FROM calendar_events WHERE date = :date AND schoolCalendarEventRefId = :schoolId")
