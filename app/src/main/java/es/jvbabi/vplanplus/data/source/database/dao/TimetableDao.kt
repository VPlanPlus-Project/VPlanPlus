...
abstract class TimetableDao {
    ...
    @Upsert
    abstract suspend fun upsertTimetable(timetable: DbTimetable)

    @Upsert
    abstract suspend fun upsertTimetableLessons(timetable: List<DbTimetable>)

    @Query("INSERT OR REPLACE INTO timetable_room_crossover (lesson_id, room_id) VALUES (:lessonId, :roomId)")
    abstract suspend fun upsertTimetableRoomCrossover(lessonId: UUID, roomId: Int)

    @Upsert
    abstract suspend fun upsertTimetableRoomCrossovers(crossovers: List<TimetableRoomCrossover>)

    @Query("INSERT OR REPLACE INTO timetable_teacher_crossover (lesson_id, school_entity_id) VALUES (:lessonId, :schoolEntityId)")
    abstract suspend fun upsertLessonTeacherCrossover(lessonId: UUID, schoolEntityId: UUID)

    @Upsert
    abstract suspend fun upsertLessonTeacherCrossovers(crossovers: List<TimetableTeacherCrossover>)

    @Query("DELETE FROM timetable WHERE class_id IN (SELECT id FROM `group` WHERE school_id = :schoolId)")
    abstract suspend fun clearTimetableForSchool(schoolId: Int)
    ...
    abstract suspend fun getTimetableForGroup(groupId: Int, weekId: Int?, weekTypeId: Int?, dayOfWeek: Int): List<CTimetable>
    ...
    abstract suspend fun getWeekTimetableForSchool(schoolId: Int, weekId: Int?, weekTypeId: Int?): List<CTimetable>

    @Query("DELETE FROM timetable WHERE id IN (:ids)")
    abstract suspend fun deleteFromTimetableById(ids: List<UUID>)
