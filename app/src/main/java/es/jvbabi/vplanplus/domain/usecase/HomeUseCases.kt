...
class HomeUseCases(
    ...
    suspend fun getLessons(profile: Profile, date: LocalDate): Flow<Day> {
        return when (profile.type) {
            ProfileType.STUDENT -> lessonRepository.getLessonsForClass(profile.referenceId, date, null)
            ProfileType.TEACHER -> lessonRepository.getLessonsForTeacher(profile.referenceId, date, null)
            ProfileType.ROOM -> lessonRepository.getLessonsForRoom(profile.referenceId, date, null)
        }.map { dayPair ->
